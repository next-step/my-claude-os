...
| 에이전트 | 역할 | 관점 |
|---|---|---|
| **product-planner** | 기획·총괄(PM/테크리드). 무엇을 어떤 순서로·어떤 스택으로 만들지, 백엔드/프론트 작업 범위를 정한다 | 상위 의사결정 |
| **backend-developer** | 서버 운용·DB 스키마·데이터 수집(API 우선 → 크롤링 → URL 폴백) 설계 | 서비스 지향 |
| **frontend-developer** | 화면 구성·화면 흐름·사용 편의성(UX) 설계 | 사용자 지향 |
...
## 협업·공유 계층 (별도 "공유 에이전트"는 없음)
서브에이전트끼리 **직접 호출은 불가능**하다. 그래서 별도의 공유 에이전트를 두는 대신, 아래 두 가지가 공유 기반 역할을 한다.
