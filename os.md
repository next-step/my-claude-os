...
현재 두 축은 수동/웹 기반이다: **매매 기록은 대화형 수기 입력**(`log-trade`, §10),
**데일리 시세는 웹 조회**(`market-scanner`, §5). 다음 단계에서 **증권사 API**로 자동화한다.

- **매매 자동 수집**: 체결 내역을 API로 받아 `holdings.md` + `trade-journal.md`에 자동 기록한다.
  → 커널 §1 "기록 없는 매매 없음"을 사람 대화가 아니라 **API 동기화로 강제**(수기 입력 오류 제거).
  구현 시 `log-trade`의 대화형 경로는 API 미체결분·수동 보정용으로 남긴다.
- **시세 자동 조회**: 데일리 시세·잔고를 API로 직접 조회해 로컬 시세 스냅샷에 기록하고 보고서 생성.
  → `market-scanner` 웹 조회 신뢰도 의존을 줄인다(웹은 뉴스·매크로 보조로 유지).
- **불변 조건**: API 연동 후에도 §6 가드레일 유지 — **조회·기록·보고서까지만** 자동화하고
  실제 주문·송금·환전은 100% 사람. API 키·조회 데이터는 **로컬 전용**(§9의 `holdings.md` 원칙과 동일).
- 반영 위치: 새 수집 역할은 §4 subagent 또는 자동화(§9), API 스펙·엔드포인트는 `05_reference/`,
  키 관리·권한은 §6·§9에 추가.
