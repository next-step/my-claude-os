...
### Step 3: writer 에이전트에 갱신 위임

oh-my-claudecode의 `writer` 에이전트(README/문서 전문, Haiku)에게 위임한다.
Agent 도구를 호출하며 아래를 전달한다.

- **목표**: README.md를 Step 1의 실제 사실과 일치하도록 갱신
- **입력 데이터**: Step 1 스캔 결과 전체 + Step 2에서 찾은 불일치 목록
- **지켜야 할 제약**:
