...
- `02_portfolio/holdings.md` — 보유·평단·손절/목표가, 이번 달 단타 누적손익
...
## 판단 규칙
- **버킷 분리**: 적립식과 단타를 섞지 않는다. 단타 손실을 적립식으로 메우는 제안 금지.
- **단타 종목**: 손절가·목표가 **도달 여부**를 종목별로 점검. 월 손실한도(-15%) **잔여액**을 계산해 알린다. 1회 300~400만·손절 -5% 규칙 적용.
- **적립식**: 정기매수일·리밸런싱 신호만 점검. 잦은 매매 유도 금지.
